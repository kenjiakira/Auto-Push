...
module.exports.run = async ({ api, event, args, Currencies }) => {
  ...
  const vaultUnlockTime = 5 * 60 * 1000; 
  ...
  if (success) {
    ...
    api.sendMessage(`Bạn đã bị cảnh sát bắt và bị phạt ${penalty} xu. Bạn cần chờ thêm 5 phút nữa để mở khóa két sắt ngân hàng lần nữa.`, threadID, messageID);
    vaultCooldown.set(senderID, currentTime + vaultUnlockTime); 
  }
};
