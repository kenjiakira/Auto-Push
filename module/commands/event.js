...
const loadEventModules = function({ moduleList, threadID, messageID }) {
  ...
  for (const nameModule of moduleList) {
    try {
      const dirModule = join(eventModuleDir, nameModule + '.js');
      ...
      if (eventModule.config.dependencies && typeof eventModule.config.dependencies === 'object') {
        ...
        for (const packageName in eventModule.config.dependencies) {
          ...
          const moduleDir = join(__dirname, 'node_modules', packageName);
          try {
...
module.exports.run = async function({ event, args, api, Users }) {
  ...
  else return loadEventModules({ moduleList, threadID, messageID });
};
